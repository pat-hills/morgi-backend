...
@section('content')
    ...
    <div class="row justify-content-center">
        <div class="col-4" style="text-align: right">
            <a href="#" type="button" class="btn btn-primary active" style="color: white"> <svg xmlns="http://www.w3.org/2000/svg" width="9" height="9" fill="currentColor" class="bi bi-circle-fill" viewBox="0 0 16 16" style="color: limegreen">
                ...
                </svg> Active subscriptions ({{count($subscriptions)}})</a>
        </div>
        <div class="col-4" style="text-align: left">
            <a href="{{route('user.edit.active_gifts.not-active-gifts', $user->id)}}" type="button" class="btn btn-primary">Inactive subscriptions ({{$opposite_subscriptions_counter}})</a>
        </div>
    </div>
    ...
    <br>
        ...
        <div class="row justify-content-center" style="background: whitesmoke">
            <div class="col-4">
                TOTAL ACTIVE MONTHLY SUBSCRIPTIONS: <span style="color: green">{{$total_monthly}} MORGI</span>
            </div>
        </div>
    <br>
                    ...
                    <tbody>
                        ...
                        @foreach($subscriptions as $key => $sub)
                            <tr data-toggle="collapse" data-target="#demo{{$sub['id']}}" class="accordion-toggle" onclick="setSubTable('{{$sub['id']}}')">
                                <td>{{$sub['username']}}</td>
                                <td>#{{$sub['username_id']}}</td>
                                <td>{{$sub['email']}}</td>
                                <td>{{$sub['last_subscription_at']}}</td>
                                <td>{{strtoupper($sub['status'])}}</td>
                                <td>{{$sub['morgi']}}</td>
                                <td><a class="btn btn-danger text-white" href="#" data-toggle="modal" data-target="#modalEndSubscription" onclick="setEndSubscription('{{$sub['id']}}')">END</a></td>
    ...
    @include('admin.admin-pages.user_profile.user_widget.modal-end-subscription')
    ...
    <script>
        ...
        function setEndSubscription(subscription){
            $('#subscription_to_end').val(subscription);
        }
    </script>
