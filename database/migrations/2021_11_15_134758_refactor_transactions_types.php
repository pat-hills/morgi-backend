...
class RefactorTransactionsTypes extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        \App\Models\TransactionType::query()->where('type', 'chat')->update([
            'description_leader' => 'Gift Micro Morgi on chat to <a href="/rookie-profile/{{rookie_id}}">{{rookie_full_name}}</a>',
            'description_rookie' => "Gift Micro Morgi on chat from {{leader_full_name}}",
        ]);

        \App\Models\TransactionType::query()->where('type', 'gift')->update([
            'description_leader' => 'Recurring monthly gift of {{morgi}} Morgis to <a href="/rookie-profile/{{rookie_id}}">{{rookie_full_name}}</a>',
        ...
        \App\Models\TransactionType::query()->where('type', 'refund')->update([
            'description_leader' => 'System refund following an error to <a href="/rookie-profile/{{rookie_id}}">{{rookie_full_name}} (#{{referal_internal_id}})</a>',
        ...
        \App\Models\TransactionType::query()->where('type', 'rookie_block_leader')->update([
            'description_leader' => 'Canceled connection with <a href="/rookie-profile/{{rookie_id}}">{{rookie_full_name}}</a> refund',
