...
class EventOpenConnectionClick extends MixpanelEventBuilder
{
    public $type = 'open_connection_click';

    public function __construct(int $user_id, array $frontend_properties)
    {
        parent::__construct($user_id, $frontend_properties);

        if(!array_key_exists('Rookie Id', $frontend_properties)){
            throw new \Exception("You must send rookie_id");
        }
        ...
        $properties = [
            ...
            'Future connection number' => $leader_profile['Total connections'] + 1,
