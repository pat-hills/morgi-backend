...
class EventGiftMorgiSuccess extends MixpanelEventBuilder
{
    public $type = 'gift_morgi_success';

    public static function config(int $user_id, int $rookie_id, float $amount, bool $is_one_click_payment): void
    {
        $leader_id = $user_id;
    ...
    public function store(): void
    {
        try {
            parent::store();
            (new EventGiftMorgiSuccessForRookie($this->user->id, $this->properties))->store();
        }catch (\Exception $exception){
            throw new \Exception($exception->getMessage());
        }
    }
